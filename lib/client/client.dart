...
class Client {
  ...
  Client({required String host, required int port}){
       _tcpBloc =  TcpBloc(readFunc: handleFunc, encryptionEnabled: true);
       _tcpBloc!.add(Connect(host: host, port: port));
  }

  void handleFunc(Uint8List data){
      int lastMethodNameIndex = getLastMethodNameIndex(data);
      String methodName = String.fromCharCodes(data.sublist(0, lastMethodNameIndex));
  ...
  GetUserResponse GetUser(GetUserRequest request){
    ...
    Uint8List rawResponse = WaitResponse(_waitResponsesMap, "GetUser", maxTimerDifference);
    GetUserResponse response = GetResponse("GetUser", rawResponse) as GetUserResponse;
    return response;
  }
}
